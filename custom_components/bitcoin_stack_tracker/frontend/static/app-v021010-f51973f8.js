...
const BUILD_VERSION = "0.21.0.10";
const FRONTEND_BUILD = "021009";
...
function renderMarketAssessmentHistory(){
  ...
  if(!points.length){host.innerHTML=`<p class="storage-note">${esc(walletWatchLang("Noch keine rekonstruierte Score-Historie geladen.","No reconstructed score history loaded yet."))}</p>`;if(hint)hint.textContent="";return;}
  ...
  const validPrices=points.map(p=>Number(p.price)).filter(v=>Number.isFinite(v)&&v>0),priceTransform=value=>priceLog?Math.log10(Number(value)):Number(value),priceInverse=value=>priceLog?10**Number(value):Number(value);
  let priceMin=0,priceMax=1;
  if(validPrices.length){const mapped=validPrices.map(priceTransform);priceMin=Math.min(...mapped);priceMax=Math.max(...mapped);if(priceMin===priceMax){const d=Math.max(Math.abs(priceMin)*.05,.05);priceMin-=d;priceMax+=d;}const pp=(priceMax-priceMin)*.06;priceMin-=pp;priceMax+=pp;if(!priceLog)priceMin=Math.max(0,priceMin);}
  const yPrice=value=>pad.t+(1-(priceTransform(value)-priceMin)/Math.max(1e-12,priceMax-priceMin))*ph;
  const scorePoly=points.map((p,i)=>`${x(i).toFixed(2)},${yScore(p.display_score).toFixed(2)}`).join(" ");
  const priceRows=overlay?points.map((p,i)=>({i,value:Number(p.price)})).filter(row=>Number.isFinite(row.value)&&row.value>0):[];
  const pricePoly=priceRows.map(row=>`${x(row.i).toFixed(2)},${yPrice(row.value).toFixed(2)}`).join(" ");
  const grid=[0,25,50,75,100].map(v=>{const yy=yScore(v);return `<line class="grid" x1="${pad.l}" y1="${yy}" x2="${width-pad.r}" y2="${yy}"/><text class="axis-text" x="8" y="${yy+4}">${v}</text>`;}).join("");
  const priceAxis=overlay?[0,.25,.5,.75,1].map(f=>{const yy=pad.t+f*ph,value=priceInverse(priceMax-f*(priceMax-priceMin));return `<text class="axis-text market-price-axis" x="${width-4}" y="${yy+4}" text-anchor="end">${esc(compactAxis(value))}</text>`;}).join(""):"";
  const tickFractions=mobile?[0,.5,1]:[0,.25,.5,.75,1];
  const nearestIndexForTime=target=>{let lo=0,hi=times.length-1;while(lo<hi){const mid=Math.floor((lo+hi)/2);if(times[mid]<target)lo=mid+1;else hi=mid;}if(lo>0&&Math.abs(times[lo-1]-target)<Math.abs(times[lo]-target))return lo-1;return lo;};
  const tickIdx=[...new Set(tickFractions.map(f=>nearestIndexForTime(minT+f*span)))],labels=tickIdx.map(i=>`<text class="date-text" x="${x(i)}" y="${height-12}" text-anchor="middle">${esc(fmtDate(points[i].date))}</text>`).join("");
  host.innerHTML=`<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="${esc(walletWatchLang("Historischer Markteinschätzungs-Score mit Bitcoin-Preis","Historical market assessment score with Bitcoin price"))}">
    ${grid}${priceAxis}<polyline class="series-primary market-score-series" fill="none" points="${scorePoly}"/>${overlay&&pricePoly?`<polyline class="series-secondary market-price-series" stroke-opacity="${priceOpacity.toFixed(2)}" fill="none" points="${pricePoly}"/>`:""}${labels}
    <text class="axis-text market-score-axis-title" x="${pad.l}" y="18">${esc(walletWatchLang(`Markteinschätzung · 0–100 · ${marketAssessmentSmoothingLabel()}`,`Market assessment · 0–100 · ${marketAssessmentSmoothingLabel()}`))}</text>${overlay?`<text class="axis-text market-price-axis-title" x="${width-pad.r}" y="18" text-anchor="end">${esc(walletWatchLang(`Bitcoin-Preis · ${currency} · ${priceLog?"logarithmisch":"linear"}`,`Bitcoin price · ${currency} · ${priceLog?"logarithmic":"linear"}`))}</text>`:""}
    <line id="marketHistoryCrossX" class="crosshair hidden" x1="0" y1="${pad.t}" x2="0" y2="${height-pad.b}"/>
    <line id="marketHistoryCrossY" class="crosshair hidden" x1="${pad.l}" y1="0" x2="${width-pad.r}" y2="0"/>
    <circle id="marketHistoryCrossDot" class="cross-dot hidden" r="5" stroke="#f7931a"/>
    <circle id="marketHistoryPriceDot" class="cross-dot hidden" r="4.5" stroke="#66d19e"/>
    <g id="marketHistoryDateBadge" class="crosshair-axis-badge hidden"><rect rx="6" ry="6" width="104" height="24"/><text x="52" y="16" text-anchor="middle">–</text></g>
    <g id="marketHistoryScoreBadge" class="crosshair-axis-badge hidden"><rect rx="6" ry="6" width="50" height="24"/><text x="25" y="16" text-anchor="middle">–</text></g>
    <g id="marketHistoryPriceBadge" class="crosshair-axis-badge market-price-badge hidden"><rect rx="6" ry="6" width="84" height="24"/><text x="42" y="16" text-anchor="middle">–</text></g>
    <rect id="marketHistoryHit" class="chart-hit" x="${pad.l}" y="${pad.t}" width="${pw}" height="${ph}"/>
  </svg>`;
  const hit=$("#marketHistoryHit",host),crossX=$("#marketHistoryCrossX",host),crossY=$("#marketHistoryCrossY",host),dot=$("#marketHistoryCrossDot",host),priceDot=$("#marketHistoryPriceDot",host),dateBadge=$("#marketHistoryDateBadge",host),scoreBadge=$("#marketHistoryScoreBadge",host),priceBadge=$("#marketHistoryPriceBadge",host);
  const hide=()=>[crossX,crossY,dot,priceDot,dateBadge,scoreBadge,priceBadge].forEach(node=>node?.classList.add("hidden"));
  const showAtPointer=event=>{
    const rect=hit.getBoundingClientRect();if(!(rect.width>0))return;
    const fraction=Math.max(0,Math.min(1,(event.clientX-rect.left)/rect.width)),index=nearestIndexForTime(minT+fraction*span),point=points[index],score=Number(point?.display_score),price=Number(point?.price);
    if(!Number.isFinite(score)){hide();return;}
    const xPos=x(index),yPos=yScore(score),dateText=fmtDate(point.date),scoreText=fmtNumber(score,1);
    crossX.setAttribute("x1",xPos);crossX.setAttribute("x2",xPos);crossX.classList.remove("hidden");
    crossY.setAttribute("y1",yPos);crossY.setAttribute("y2",yPos);crossY.classList.remove("hidden");
    dot.setAttribute("cx",xPos);dot.setAttribute("cy",yPos);dot.classList.remove("hidden");
    const dateWidth=104,dateX=Math.max(pad.l,Math.min(width-pad.r-dateWidth,xPos-dateWidth/2));dateBadge.setAttribute("transform",`translate(${dateX},${height-pad.b+8})`);$("text",dateBadge).textContent=dateText;dateBadge.classList.remove("hidden");
    const scoreY=Math.max(pad.t,Math.min(height-pad.b-24,yPos-12));scoreBadge.setAttribute("transform",`translate(4,${scoreY})`);$("text",scoreBadge).textContent=scoreText;scoreBadge.classList.remove("hidden");
    if(overlay&&Number.isFinite(price)&&price>0){const py=yPrice(price);priceDot.setAttribute("cx",xPos);priceDot.setAttribute("cy",py);priceDot.classList.remove("hidden");const badgeWidth=84,priceY=Math.max(pad.t,Math.min(height-pad.b-24,py-12));priceBadge.setAttribute("transform",`translate(${width-badgeWidth-3},${priceY})`);$("text",priceBadge).textContent=fmtFiat(price,currency);priceBadge.classList.remove("hidden");}else{priceDot.classList.add("hidden");priceBadge.classList.add("hidden");}
  };
  hit.addEventListener("pointerleave",hide);hit.addEventListener("pointermove",showAtPointer);hit.addEventListener("pointerdown",showAtPointer);
...
$("#portfolioSelect").onchange=async event=>{state.entryId=event.target.value;state.halvings=[];state.halvingInfo=null;state.halvingsEntryId="";state.halvingsError="";state.walletWatch=null;state.walletWatchLoading=false;bitcoinNetworkRefreshAt=0;localStorage.setItem("bst_entry",state.entryId);await loadData();await loadTorRotationSettings();if(state.activeTab==="walletwatch"&&!state.data?.locked)await loadWalletWatch();};
...
function renderWalletWatch(){
  ...
  list.innerHTML=(cfg.monitors||[]).length?(cfg.monitors||[]).map(mon=>{
    const safeValue=maskWalletWatchValue(mon.value);const reserve=mon.kind==="address"?walletWatchLang("Exakte Adresse dauerhaft überwachen","Monitor this exact address permanently"):`Receive ${mon.receive_count||0} · Change ${mon.change_count||0}`;
...
function renderWalletWatchActivity(){
  ...
  const rowHtml=item=>{const outgoing=item.direction==="outgoing",direction=outgoing?walletWatchLang("Ausgang","Outgoing"):walletWatchLang("Eingang","Incoming"),status=item.confirmed?walletWatchLang("Bestätigt","Confirmed"):walletWatchLang("Unbestätigt","Unconfirmed"),amount=state.discreet?"••••":`${fmtNumber(Number(item.amount_sats||0)/SATsFix(),8)} BTC`,watched=walletWatchWatchedPartyHtml(item,log),other=walletWatchCounterpartyHtml(item,{log}),sender=outgoing?watched:other,recipient=outgoing?other:watched;return `<tr><td>${esc(fmtDateTime(item.detected_at))}</td><td><span class="sats-sentinel-category ${esc(walletWatchCategoryClass(item.category))}">${esc(walletWatchCategoryLabel(item.category))}</span></td><td>${sender}</td><td class="sats-sentinel-flow-direction ${outgoing?"negative":"positive"}"><span>${esc(direction)}</span><span aria-hidden="true">→</span></td><td>${recipient}</td><td>${esc(amount)}</td><td>${esc(status)}${item.rbf?" · RBF":""}</td><td>${walletWatchTxLink(item,log)}</td></tr>`;};
  ...
  if(cards)cards.innerHTML=items.map(item=>{const outgoing=item.direction==="outgoing",direction=outgoing?walletWatchLang("Ausgang","Outgoing"):walletWatchLang("Eingang","Incoming"),amount=state.discreet?"••••":`${fmtNumber(Number(item.amount_sats||0)/SATsFix(),8)} BTC`,watched=walletWatchWatchedPartyHtml(item,log),other=walletWatchCounterpartyHtml(item,{mobile:true,log}),sender=outgoing?watched:other,recipient=outgoing?other:watched;return `<article class="ledger-mobile-card sats-sentinel-activity-card"><div class="ledger-mobile-card-head"><strong>${esc(direction)} · ${esc(amount)}</strong><span>${esc(fmtDateTime(item.detected_at))}</span></div><div class="sats-sentinel-mobile-flow"><section><span>${esc(walletWatchLang("Sender","Sender"))}</span>${sender}</section><div class="sats-sentinel-mobile-direction ${outgoing?"negative":"positive"}"><strong>${esc(direction)}</strong><span aria-hidden="true">→</span></div><section><span>${esc(walletWatchLang("Empfänger","Recipient"))}</span>${recipient}</section></div><div><span>TXID</span>${walletWatchTxLink(item,log)}</div></article>`;}).join("");
...
function resetWalletWatchMonitorForm(){const form=$("#walletWatchAddForm");if(!form)return;form.reset();form.elements.edit_id.value="";form.elements.kind.value="address";form.elements.category.value="other";form.elements.receive_count.value="2";form.elements.change_count.value="2";form.elements.min_notify_amount.value="0";form.elements.min_notify_unit.value="sats";for(const name of ["notify_incoming","notify_outgoing","notify_ha_event","notify_persistent","notify_services","notify_external"])form.elements[name].checked=true;$("#walletWatchMonitorSubmit").textContent=walletWatchLang("Zur Überwachung hinzufügen","Add to monitoring");$("#walletWatchMonitorCancel").classList.add("hidden");syncWalletWatchKindUi();}
function editWalletWatchMonitor(id){const cfg=walletWatchConfig(),mon=(cfg.monitors||[]).find(item=>item.id===id),form=$("#walletWatchAddForm");if(!mon||!form)return;form.elements.edit_id.value=mon.id;form.elements.label.value=mon.label||"";form.elements.category.value=mon.category||"other";form.elements.kind.value=mon.kind||"address";form.elements.value.value=mon.value||"";form.elements.receive_count.value=String(mon.receive_count||0);form.elements.change_count.value=String(mon.change_count||0);form.elements.note.value=mon.note||"";form.elements.min_notify_unit.value="sats";form.elements.min_notify_amount.value=String(mon.min_notify_sats||0);for(const name of ["notify_incoming","notify_outgoing","notify_ha_event","notify_persistent","notify_services","notify_external"])form.elements[name].checked=mon[name]!==false;$("#walletWatchMonitorSubmit").textContent=walletWatchLang("Änderungen übernehmen","Apply changes");$("#walletWatchMonitorCancel").classList.remove("hidden");syncWalletWatchKindUi();form.scrollIntoView({behavior:"smooth",block:"start"});}
function SATsFix(){return 100000000;}
async function loadWalletWatch(){
  if(!state.entryId||state.data?.locked||!state.data?.security?.owner)return;
  state.walletWatchLoading=true;renderWalletWatch();
  try{state.walletWatch=await api(`api/wallet-watch?entry_id=${encodeURIComponent(state.entryId)}`,{timeoutMs:30000});}
...
async function removeWalletWatchMonitor(id){
  ...
  const cfg=walletWatchDraftConfig();cfg.monitors=(cfg.monitors||[]).filter(item=>item.id!==id);const previous=state.walletWatch;
  try{const response=await api("api/wallet-watch",{method:"POST",body:JSON.stringify({entry_id:state.entryId,config:cfg}),timeoutMs:30000});state.walletWatch=response;resetWalletWatchMonitorForm();renderWalletWatch();await loadWalletWatchActivity(1);const purged=Number(response?.status?.purged_activity_count||0);toast(walletWatchLang(`Watch-Eintrag entfernt · ${purged} Journal-Eintrag${purged===1?"":"e"} dauerhaft gelöscht.`,`Watch entry removed · ${purged} journal entr${purged===1?"y":"ies"} permanently deleted.`));}
  catch(error){state.walletWatch=previous;renderWalletWatch();toast(errorText(error));}
}
function addWalletWatchMonitor(event){
  event.preventDefault();if(!state.walletWatch)return;const form=event.currentTarget,fd=new FormData(form),kind=String(fd.get("kind")||"address"),value=String(fd.get("value")||"").trim();if(!value){toast(walletWatchLang("Adresse/XPUB/Descriptor fehlt.","Address/XPUB/descriptor is missing."));return;}
  const cfg=walletWatchConfig(),editId=String(fd.get("edit_id")||""),unit=String(fd.get("min_notify_unit")||"sats"),rawAmount=Math.max(0,Number(fd.get("min_notify_amount")||0)),minNotifySats=Math.round(unit==="btc"?rawAmount*SATsFix():rawAmount);cfg.monitors=cfg.monitors||[];const existing=editId?cfg.monitors.find(item=>item.id===editId):null,id=existing?.id||`watch_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,7)}`;const monitor={id,label:String(fd.get("label")||"").trim()||existing?.label||`Wallet ${cfg.monitors.length+1}`,category:String(fd.get("category")||"other"),note:String(fd.get("note")||"").trim(),kind,value,enabled:existing?.enabled!==false,receive_count:kind==="address"?0:Number(fd.get("receive_count")||2),change_count:kind==="address"?0:Number(fd.get("change_count")||2),min_notify_sats:minNotifySats,notify_incoming:form.elements.notify_incoming.checked,notify_outgoing:form.elements.notify_outgoing.checked,notify_ha_event:form.elements.notify_ha_event.checked,notify_persistent:form.elements.notify_persistent.checked,notify_services:form.elements.notify_services.checked,notify_external:form.elements.notify_external.checked};if(existing){cfg.monitors=cfg.monitors.map(item=>item.id===id?monitor:item);}else cfg.monitors.push(monitor);state.walletWatch.config=cfg;resetWalletWatchMonitorForm();renderWalletWatch();toast(existing?walletWatchLang("Eintrag geändert – zum Aktivieren noch Sats Sentinel speichern.","Entry changed – save Sats Sentinel to activate."):walletWatchLang("Hinzugefügt – zum Aktivieren noch speichern.","Added – save to activate."));
}
