...
const BUILD_VERSION = "0.21.0.6";
const FRONTEND_BUILD = "021006-733b783d";
...
Object.assign(I18N.en, {
  stackingSpeed:"Stacking speed",stackingSpeedHint:"Net BTC change including purchases, opening-balance additions, sales, and expenses.",last30Days:"Last 30 days",last365Days:"Last 365 days",sinceStart:"Since start",avgPerDay:"Avg per day",avgPerMonth:"Avg per month",
  feeAnalytics:"Fees",feeAnalyticsHint:"Fee ratios are volume-weighted, so they remain comparable across differently sized bookings. The disposition fee ratio includes sales and priced expenses/card payments. BTC fees include fees actually charged in BTC, including on-chain/mining fees; incomplete legacy data is never presented as a false zero.",totalFees:"Total fees",btcFees:"BTC fees (total)",purchaseFeeRate:"Purchase fee ratio",dispositionFeeRate:"Disposition fee ratio",weightedByVolume:"volume-weighted",btcFeeIncludesOnchain:"includes on-chain/mining fees",btcFeeLegacyIncomplete:"older on-chain bookings without a reconstructable BTC fee are present",
  hodlBenchmark:"Cash-flow-neutral HODL benchmark",hodlBenchmarkHint:"Compares the current stack with a fee-free HODL path using the same external fiat cash flows. Purchases are treated as fiat inflows and sales/expenses as fiat withdrawals; no hidden FX rate is invented for mixed-fiat ledgers.",hodlStack:"HODL stack",yourStack:"Your stack",difference:"Difference",strategyVsHodl:"Strategy vs HODL",hodlUnavailableMixedFiat:"Unavailable: the ledger contains multiple fiat currencies and the tracker does not invent an FX rate.",
  btcCagr:"BTC CAGR since first entry",btcCagrHint:"Annualized BTC market-price change since the first priced ledger entry. It is not personal return; use XIRR and TWR for that.",
  netInvestedFiat:"Net fiat invested",netInvestedFiatHint:"Purchase outlays including purchase fees minus net proceeds from sales. Expenses are not treated as fiat returned to you.",realizedHint:"Completed FIFO sales after assigned cost basis and sale fees.",unrealizedHint:"Current book profit/loss of open FIFO lots versus cost basis including allocated purchase fees.",totalProfitHint:"Realized plus unrealized profit/loss.",
...
function fifoMetricEvents(currency) {
  ...
  for (const item of entries) {
    ...
    if (!cursorByDepot.has(depot)) cursorByDepot.set(depot,0);
    if (kind === "purchase" || kind === "stack") {
      if (kind === "purchase") {
        ...
        if (entryCurrency === selectedCurrency && Number.isFinite(unitBasis) && unitBasis >= 0) {
          basis += amount * unitBasis;
          knownBtc += amount;
        }
      } else {
      ...
      let remaining = amount, cursor = Number(cursorByDepot.get(depot) || 0);
      ...
      while (cursor < lots.length && remaining > 1e-15) {
        ...
        if (!(lot.remaining > 1e-15)) { cursor += 1; continue; }
        ...
        if (lot.currency === selectedCurrency && Number.isFinite(lot.unitBasis) && lot.unitBasis >= 0) {
          basis -= take * lot.unitBasis;
          knownBtc -= take;
          if (kind === "sale" && saleCurrency === selectedCurrency && Number.isFinite(salePrice) && salePrice > 0) {
        ...
        lot.remaining -= take;
        remaining -= take;
        if (!(lot.remaining > 1e-15)) cursor += 1;
      }
...
function updateChartScaleButtons(series = []) {
  for (const index of [0,1]) {
    ...
    const logBlocked = Boolean(item?.allowNegative);
    ...
    button.title = logBlocked ? `${axisLabel}: ${t("logUnavailable")}` : `${axisLabel}: ${label}`;
...
function renderChart() {
  ...
  const rightAxis = overlay ? [0,.25,.5,.75,1].map(fraction => { const yy=pad.t+fraction*plotHeight,value=axisValue(extents[1],fraction,1); return `<text class="axis-text" x="${width-4}" y="${yy+4}" text-anchor="end">${privateHtml(compactAxis(value))}</text>`; }).join("") : "";
  ...
  hit.addEventListener("pointermove",event=>{
    ...
    tooltip.innerHTML=`<strong>${esc(fmtChartPoint(day,intradayAxis))}</strong><div><span class="chart-tooltip-dot primary" aria-hidden="true"></span>${esc(series[0].label)}: ${privateHtml(series[0].format(primary))}</div>${overlay&&usable(secondaryValue,1)?`<div><span class="chart-tooltip-dot secondary" aria-hidden="true"></span>${esc(series[1].label)}: ${privateHtml(series[1].format(secondaryValue))}</div>`:""}${markerDetails}`;
...
function csvDuplicatePayloadRow(row){
  return {
    ...
    currency:row.currency,price:row.price,fee:row.fee||0,fee_btc:row.fee_btc||0,
...
function renderCsvImportPreview() {
  ...
  $$("#csvImportBody [data-field]").forEach(input => {
    const handler=event=>{
      ...
      csvRowsFromModal();
      refreshCsvReviewVisuals();
      if(["type","timestamp","amount_btc","amount_unit","currency","price","fiat_total","fee","depot_id"].includes(fieldName))scheduleCsvDuplicateCheck();
    };
...
async function confirmCsvImport() {
  ...
  try{
    const result=await service("bulk_import",{
      ...
      transactions:rows.map(row=>({type:row.type,timestamp:row.timestamp,amount_btc:row.amount_btc,currency:row.currency,price:row.price,fee:row.fee,depot_id:row.depot_id,note:csvComposedNote(row),fee_btc:row.fee_btc||0,import_ref_hash:row.import_ref_hash||""}))
    },{timeoutMs:300000});
...
function xirrAnalysis(currency) {
  ...
  const fxRequired=chartLedgerEntries().some(entry=>{
    ...
    return Number.isFinite(time)&&time>startTime&&time<=endTime&&["purchase","sale","expense"].includes(kind)&&entryCurrency&&entryCurrency!==selectedCurrency;
  });
...
function renderFeeAnalytics(currency){
  ...
  element.innerHTML=[
    analysisCard(t("totalFees"),fmtFiat(Number(fees.total_fiat||0),currency),`${esc(t("feeRatio"))}: ${privateHtml(`${fmtNumber(Number(fees.ratio_percent||0),3)} %`)}`),
