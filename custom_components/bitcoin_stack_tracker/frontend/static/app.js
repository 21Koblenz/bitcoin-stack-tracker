...
function walletWatchMonitorRuntimeSummary(monitorId){
  const id=String(monitorId||""),addresses=Array.isArray(state.walletWatch?.status?.addresses)?state.walletWatch.status.addresses.filter(row=>String(row?.monitor_id||"")===id):[];
  const last=state.walletWatch?.status?.last_activity_by_monitor?.[id]||null;
  return {addresses:addresses.length,balance_sats:addresses.reduce((sum,row)=>sum+Number(row?.balance_sats||0),0),utxo_count:addresses.reduce((sum,row)=>sum+Number(row?.utxo_count||0),0),baseline_complete:addresses.length>0&&addresses.every(row=>Boolean(row?.baseline_complete)),last_activity:last};
}
...
async function addWalletWatchMonitor(event){
  ...
  try{const response=await api("api/wallet-watch/upsert-monitor",{method:"POST",body:JSON.stringify({entry_id:state.entryId,monitor}),timeoutMs:30000});const previous=state.walletWatch;state.walletWatch={...previous,config:response.config,status:response.status,notify_services:response.notify_services||previous.notify_services||[],activity_log:response.activity_log||previous.activity_log};delete state.walletWatchTxOverviews[id];resetWalletWatchMonitorForm();renderWalletWatch();if(result){result.textContent=existing?walletWatchLang("Watch-Eintrag gespeichert.","Watch entry saved."):walletWatchLang("Watch-Eintrag hinzugefügt und gespeichert.","Watch entry added and saved.");result.className="result positive";}toast(existing?walletWatchLang("Watch-Eintrag gespeichert.","Watch entry saved."):walletWatchLang("Watch-Eintrag hinzugefügt und gespeichert.","Watch entry added and saved."));}
  catch(error){const message=errorText(error);if(result){result.textContent=message;result.className="result negative";}toast(message);}finally{if(button)button.disabled=false;}
}
