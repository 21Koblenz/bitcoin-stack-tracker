...
function btcToDisplayedAmount(value, unit) {
  ...
  return String(unit || "BTC").toLowerCase() === "sats" ? btc * SATS_PER_BTC : btc;
}
...
function transactionFiatTotal(type, amountBtc, price, fee=0) {
  ...
  if (type === "sale") return gross - charge;
  return gross;
}
function transactionPriceFromTotal(type, amountBtc, fiatTotal, fee=0) {
  ...
  const gross = type === "purchase" ? total - charge : type === "sale" ? total + charge : total;
...
function markCsvDuplicates(rows) {
  const existing = new Set((state.data?.entries || [])
    .filter(item => ["purchase","sale","expense"].includes(item.type))
    .map(item => csvFingerprint(item)));
  const pending = new Set();
  rows.forEach(row => {
    const fingerprint = csvFingerprint(row);
    row.duplicate = Boolean(fingerprint && (existing.has(fingerprint) || pending.has(fingerprint)));
    if (row.duplicate) row.selected = false;
    if (fingerprint) pending.add(fingerprint);
  });
}
...
async function confirmCsvImport() {
  ...
  try{
    const result=await service("bulk_import",{
      ...
      transactions:rows.map(row=>({type:row.type,timestamp:row.timestamp,amount_btc:row.amount_btc,currency:row.currency,price:row.price,fee:row.fee,depot_id:row.depot_id,note:csvComposedNote(row)}))
    },{timeoutMs:120000});
...
function entryExternalFlow(entry, currency, priceSeries) {
  ...
  if (entry.type === "expense") return Number.isFinite(marketPrice) ? -(amount * marketPrice) : 0;
  return 0;
}
